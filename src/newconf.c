...
static void
conf_set_channel_caps_threshold(void *data)
{
	ConfigChannel.caps_threshold = *(unsigned int *) data;

	if(ConfigChannel.caps_threshold < 1)
	{
		conf_report_error
			("channel::caps_threshold is an absurdly low value of %d -- using 1%.",
			 ConfigChannel.caps_threshold);
			 ConfigChannel.caps_threshold = 1;
	}
	else if(ConfigChannel.caps_threshold > 100)
	{
		conf_report_error
			("channel::caps_threshold is an absurdly high value of %d -- using 100%.",
			 ConfigChannel.caps_threshold);
			 ConfigChannel.caps_threshold = 100;
	}
}
